...
export default function SearchableTable() {
  ...
  const handleEdit = async (Id) => {
    try {
      const response = await fetch(`/api/export/external/list/${Id}`);
      if (!response.ok) {
  ...
  const handleUpdate = async (Id) => {
    ...
    try {
      const response = await fetch(`/api/export/external/edit/${Id}`, {
        method: 'PUT',
  ...
  const handleDelete2 = async (Id) => {
    ...
    try {
      const response = await fetch(`/api/export/external/delete/${Id}`, {
        method: 'DELETE',
